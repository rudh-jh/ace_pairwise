...
def merge_esm(
    ...
    merged = tmp[["sequence"]].merge(esm_df, on="sequence", how="left")

    missing = merged.isna().any(axis=1).sum()
    if missing > 0:
        missing_seqs = merged.loc[merged.isna().any(axis=1), "sequence"].head(10).tolist()
        raise ValueError(
            f"Missing ESM embeddings for {missing} rows. Examples: {missing_seqs}"
        )
...
def run_one_seed(
    ...
    X_train = merge_esm(train_df, esm_df)
    X_valid = merge_esm(valid_df, esm_df)
    X_test = merge_esm(test_df, esm_df)
    ...
    for model_short_name, builder in model_builders.items():
        model_name = f"ESM2-{model_short_name}"
    ...
    result_df.to_csv(seed_out / "plm4ace_like_cls_results.csv", index=False, encoding="utf-8-sig")
    pred_valid.to_csv(seed_out / "plm4ace_like_valid_predictions.csv", index=False, encoding="utf-8-sig")
    pred_test.to_csv(seed_out / "plm4ace_like_test_predictions.csv", index=False, encoding="utf-8-sig")
...
def main() -> None:
    ...
    parser.add_argument(
        "--out_root",
        default="artifacts/classification/plm4ace_like",
    )
    ...
    if not esm_path.exists():
        raise FileNotFoundError(
            f"ESM feature file not found: {esm_path}\n"
            f"Please run scripts/33_extract_esm2_cls_embeddings.py first."
        )
    ...
    all_df.to_csv(out_dir / "plm4ace_like_cls_all_seed_results.csv", index=False, encoding="utf-8-sig")

    summary = summarize_results(all_df)
    summary.to_csv(out_dir / "plm4ace_like_cls_summary.csv", index=False, encoding="utf-8-sig")
