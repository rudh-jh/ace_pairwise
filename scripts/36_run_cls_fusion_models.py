...
def build_esm_frame(df: pd.DataFrame, esm_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing > 0:
        examples = merged.loc[merged.isna().any(axis=1), "sequence"].head(10).tolist()
...
def build_descriptor_frame_prefixed(df: pd.DataFrame) -> pd.DataFrame:
    return prefix_columns(build_descriptor_frame(df, sequence_col="sequence"), "desc")


def build_a1flat_frame_prefixed(df: pd.DataFrame) -> pd.DataFrame:
    return prefix_columns(build_a1_flat_feature_frame(df, sequence_col="sequence"), "a1")


def build_fusion_feature(
    df: pd.DataFrame,
    esm_df: pd.DataFrame,
    feature_set: str,
) -> pd.DataFrame:
    ...
    if feature_set in ["Descriptor_ESM2", "Descriptor_A1FlatFull_ESM2"]:
        parts.append(build_descriptor_frame_prefixed(df))

    if feature_set in ["A1FlatFull_ESM2", "Descriptor_A1FlatFull_ESM2"]:
        parts.append(build_a1flat_frame_prefixed(df))

    esm_part = prefix_columns(build_esm_frame(df, esm_df), "esm")
    parts.append(esm_part)
...
def fit_lr(X_train, y_train, sample_weight, seed):
    model = Pipeline(
        [
...
def fit_svc(X_train, y_train, sample_weight, seed):
    model = Pipeline(
        [
            ("imputer", SimpleImputer(strategy="median")),
            ("scaler", StandardScaler()),
    ...
    kwargs = {}
    if sample_weight is not None:
        kwargs["svc__sample_weight"] = sample_weight
...
def fit_rf(X_train, y_train, sample_weight, seed):
    model = Pipeline(
        [
            ("imputer", SimpleImputer(strategy="median")),
            (
    ...
    kwargs = {}
    if sample_weight is not None:
        kwargs["rf__sample_weight"] = sample_weight
...
def fit_mlp(X_train, y_train, sample_weight, seed):
    model = Pipeline(
        [
            ("imputer", SimpleImputer(strategy="median")),
            ("scaler", StandardScaler()),
...
def fit_catboost(X_train, y_train, sample_weight, seed):
    if not HAS_CATBOOST:
...
def run_one_seed(
    ...
    for feature_set in feature_sets:
        ...
        X_train = build_fusion_feature(train_df, esm_df, feature_set)
        X_valid = build_fusion_feature(valid_df, esm_df, feature_set)
        X_test = build_fusion_feature(test_df, esm_df, feature_set)
        ...
        for model_short_name, builder in model_builders.items():
            model_name = f"{feature_set}-{model_short_name}"
            print(f"Training {model_name}...")

            try:
                model, params = builder(X_train, y_train, sample_weight, seed)
    ...
    result_df.to_csv(seed_out / "fusion_cls_results.csv", index=False, encoding="utf-8-sig")
    pred_test.to_csv(seed_out / "fusion_cls_test_predictions.csv", index=False, encoding="utf-8-sig")
...
def main() -> None:
    ...
    parser.add_argument(
        "--out_root",
        default="artifacts/classification/fusion_models",
    )
    ...
    if not esm_path.exists():
        raise FileNotFoundError(f"ESM2 feature file not found: {esm_path}")
    ...
    all_df.to_csv(out_dir / "fusion_cls_all_seed_results.csv", index=False, encoding="utf-8-sig")

    summary = summarize_results(all_df)
    summary.to_csv(out_dir / "fusion_cls_summary.csv", index=False, encoding="utf-8-sig")
